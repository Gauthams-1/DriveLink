...
function TruckConfirmationContent() {
  ...
  useEffect(() => {
    if (truck && rentalDays > 0) {
      setTotalCost(rentalDays * truck.pricePerDay);
    } else {
      setTotalCost(0);
    }
  ...
  if (!truck) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Invalid Truck Details</CardTitle>
        </CardHeader>
        <CardContent>
          <p>We couldn't find the details for this truck. Please try again.</p>
          <Button onClick={() => router.push('/trucks')} className="mt-4">Go to Truck Listings</Button>
        </CardContent>
      </Card>
    );
  }
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                 ...
                 <div className="space-y-2">
                    ...
                    <Input id="pickupAddress" name="pickupAddress" placeholder="e.g., Warehouse A, Mumbai" required />
                 </div>
                 <div className="space-y-2">
                    <Label htmlFor="dropoffAddress">Dropoff Address</Label>
                    <Input id="dropoffAddress" name="dropoffAddress" placeholder="e.g., Client Site, Pune" required />
                 </div>
            </CardContent>
          </Card>
        </div>
        <Card>
          ...
          <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                  <span className="flex items-center gap-2 text-muted-foreground"><Calendar className="h-4 w-4"/> Booking Dates</span>
              ...
              <div className="flex items-center justify-between text-muted-foreground">
                ...
                <span>₹{truck.pricePerDay.toLocaleString()}</span>
              </div>
               <div className="flex items-center justify-between text-muted-foreground">
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-6xl">
       <div className="text-center mb-12">
            ...
            Finalize your truck booking and get the job done.
            </p>
        </div>
      <Suspense fallback={<div>Loading booking details...</div>}>
        <TruckConfirmationContent />
      </Suspense>
    </div>
  );
}
